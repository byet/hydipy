...
class Test_DiscreteNOde(unittest.TestCase):
    def test_no_parents(self):
        self.discnode = DiscreteNode(
            "a", [[0.7], [0.2], [0.1]], parents=[], states=["a1", "a2", "a3"]
        )
        self.assertEqual("a", self.discnode.id)
        self.assertIsNone(
            np.testing.assert_array_equal(
                np.array([[0.7], [0.2], [0.1]]), self.discnode.values
            )
        )
        ...
        self.assertIsNone(
            np.testing.assert_array_equal(
                np.array([0.7, 0.2, 0.1]), self.discnode.agrum_cpd()
            )
        )
    ...
    def test_one_parent(self):
        ...
        self.b = DiscreteNode(
            "b", [[0.4, 0.2, 0.9], [0.6, 0.8, 0.1]], parents=["a"], states=["b1", "b2"]
        )
        self.assertEqual("b", self.b.id)
        self.assertIsNone(
            np.testing.assert_array_equal(
                np.array([[0.4, 0.2, 0.9], [0.6, 0.8, 0.1]]), self.b.values
            )
        )
        ...
        self.assertIsNone(
            np.testing.assert_array_equal(np.array([0.7, 0.2, 0.1]), self.a.agrum_cpd())
        )
        self.assertIsNone(
            np.testing.assert_array_equal(
                np.array([0.4, 0.6, 0.2, 0.8, 0.9, 0.1]), self.b.agrum_cpd()
            )
        )
        bn = gum.BayesNet("test_bn")
    ...
    def test_two_parents(self):
        ...
        self.c = DiscreteNode(
            "c",
            [[0.4, 0.8, 0.2, 0.9, 0.25, 0.99], [0.6, 0.2, 0.8, 0.1, 0.75, 0.01]],
            parents=["a", "b"],
            states=["c1", "c2"],
        )
        ...
        self.assertIsNone(
            np.testing.assert_array_equal(np.array([0.2, 0.8]), self.b.agrum_cpd())
        )
        self.assertIsNone(
            np.testing.assert_array_equal(
                np.array(
                    [0.4, 0.6, 0.8, 0.2, 0.2, 0.8, 0.9, 0.1, 0.25, 0.75, 0.99, 0.01]
                ),
                self.c.agrum_cpd(),
            )
        )
