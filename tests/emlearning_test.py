...
class Test_DiscreteNode(unittest.TestCase):
    ...
    def test_missing_variables(self):
        ...
        self.assertDictEqual(
            eml.missing_variables(), {"b": ["b"], "c": ["c", "a", "b"]}
        )

    def test_init_full_counts(self):
        eml = EMLearner(self.model, self.train_data)
        expected = {
            "a": np.array([[2.01], [1.01]]),
            "b": np.array([[1.01], [1.01]]),
            "c": np.array([[0.01, 0.01, 0.01, 0.01], [1.01, 0.01, 0.01, 0.01]]),
        }
    ...
    def test_get_posterior(self):
        ...
        expected = np.array(
            [[0.028, 0.252, 0.036, 0.045], [0.252, 0.168, 0.084, 0.135]]
        )
    ...
    def test_e_step(self):
        eml = EMLearner(self.model, self.train_data)
        counts = eml.init_full_counts()
        outcome, log_prob = eml.e_step(counts)
        expected = {
            "a": np.array([[2.01], [1.01]]),
            "b": np.array([[1.393561644], [1.626438356]]),
            "c": np.array(
                [[0.01, 0.61, 0.01, 0.01], [1.01, 0.41, 0.393561644, 0.626438356]]
            ),
        }
        self.assertIsNone(
            np.testing.assert_array_almost_equal(expected["a"], outcome["a"])
        )
        self.assertIsNone(
            np.testing.assert_array_almost_equal(expected["b"], outcome["b"])
        )
        self.assertIsNone(
            np.testing.assert_array_almost_equal(expected["c"], outcome["c"])
        )
        self.assertTrue(expected.keys() == outcome.keys())
        self.assertAlmostEqual(log_prob, -2.386184117)

    def test_m_step(self):
        eml = EMLearner(self.model, self.train_data)
        counts = eml.init_full_counts()
        e_counts, _ = eml.e_step(counts)
        outcome = eml.m_step(e_counts)

        a = DiscreteNode(
            id="a",
            values=[[0.665562914], [0.334437086]],
            parents=[],
            states=["a1", "a2"],
        )
        b = DiscreteNode(
            id="b",
            values=[[0.461444253], [0.538555747]],
            parents=[],
            states=["b1", "b2"],
        )
        c = DiscreteNode(
    ...
    def test_em(self):
        ...
        b = DiscreteNode(
            id="b",
            values=[[0.499419889], [0.500580111]],
            parents=[],
            states=["b1", "b2"],
        )
        c = DiscreteNode(
    ...
    def test_log_prob(self):
        eml = EMLearner(self.model, self.train_data)
        outcome = eml.log_likelihood_data(self.train_data)
        expected = -3.764510308
        self.assertAlmostEqual(outcome, expected)

        outcome2 = eml.log_likelihood_data(eml.missing_data)
        expected2 = -2.386184117
        self.assertAlmostEqual(outcome2, expected2)
