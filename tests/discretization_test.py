...
class Test_Discretization(unittest.TestCase):
    ...
    def test_set_discretization(self):
        ...
        self.assertIsNone(
            np.testing.assert_array_equal(
                self.contnode.disc, np.array([-1, -0.5, 0.5, 1])
            )
        )
        self.assertIsNone(
            np.testing.assert_array_equal(self.contnode.lb, np.array([-1, -0.5, 0.5]))
        )
        self.assertIsNone(
            np.testing.assert_array_equal(self.contnode.ub, np.array([-0.5, 0.5, 1]))
        )
        self.assertIsNone(
            np.testing.assert_array_equal(
                self.contnode.median, np.array([-0.75, 0.0, 0.75])
            )
        )
    ...
    def test_discretized_pmb(self):
        ...
        self.contnode.set_discretization(np.array([-1, -0.5, 0.5, 1]))
        probs = self.contnode.discretized_probability_mass(
            dist, self.contnode.lb, self.contnode.ub
        )
        self.assertIsNone(
    ...
    def test_compute_error(self):
        ...
        error_computed = self.contnode.compute_error(
            probs, self.contnode.lb, self.contnode.ub
        )
        self.assertIsNone(
            np.testing.assert_array_almost_equal(error_computed, entropy_error)
        )
    ...
    def test_set_evidence(self):
        ...
        self.assertIsNone(
            np.testing.assert_array_equal(
                self.contnode.disc, np.array([-1, -0.5, 0.5, 0.7425, 0.7575, 1])
            )
        )
        ...
        self.assertIsNone(
            np.testing.assert_array_equal(
                self.contnode.disc, np.array([-1, -0.5, 0.495, 0.505, 1])
            )
        )
        ...
        self.assertIsNone(
            np.testing.assert_array_equal(
                self.contnode.disc, np.array([-1.01, -0.99, -0.5, 0.5, 1])
            )
        )
        ...
        self.assertIsNone(
            np.testing.assert_array_equal(
                self.contnode.disc, np.array([-1, -0.5, 0.5, 0.99, 1.01])
            )
        )
        ...
        self.assertIsNone(
            np.testing.assert_array_equal(
                self.contnode.disc, np.array([-1, -0.5, 0.5, 1, 1.98, 2.02])
            )
        )
        ...
        self.assertIsNone(
            np.testing.assert_array_equal(
                self.contnode.disc, np.array([-2.02, -1.98, -1, -0.5, 0.5, 1])
            )
        )
    ...
    def test_add_interval(self):
        ...
        self.assertIsNone(
            np.testing.assert_array_equal(new_disc, np.array([-1, -0.5, 0.0, 0.5, 1]))
        )

        new_disc = self.contnode.add_interval(np.array([-1, -0.5, 0.5, 1]), index=2)
        self.assertIsNone(
            np.testing.assert_array_equal(new_disc, np.array([-1, -0.5, 0.5, 0.75, 1]))
        )

    def test_merge_interval(self):
        new_disc = self.contnode.merge_interval(np.array([-1, -0.5, 0.5, 1]), index=1)
        self.assertIsNone(
            np.testing.assert_array_equal(new_disc, np.array([-1, 0.5, 1]))
        )
        ...
        self.assertIsNone(
            np.testing.assert_array_equal(new_disc, np.array([-1, -0.5, 0.5]))
        )
    ...
    def test_update_intervals(self):
        ...
        self.assertIsNone(
            np.testing.assert_array_equal(new_disc, np.array([-1, -0.75, -0.5, 0.5, 1]))
        )
        ...
        self.assertIsNone(
            np.testing.assert_array_equal(
                new_disc, np.array([-1, -0.5, 0.5, 1, 3, 5, 10])
            )
        )
